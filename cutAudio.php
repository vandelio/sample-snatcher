...
$output_file_name = generateRandomString();


//if(isset($_POST['filesource']) && file_exists($_POST['filesource']) && $_POST['newname'] !== '' && $_POST['start'] !== '' && $_POST['end'] !== ''){
//    $file = 'http://localhost:8888/YouTube-Downloader/Portishead-Only-You.mp3';//$_POST['filesource'];
    
           
    $file = DOMAIN_NAME.$_POST['filesource']; // realpath("input/Portishead-Only-You.mp3");
    $dest = 'output/'.$output_file_name.'_'.$_POST['newname']; // realpath('output/portissample.mp3');
    
    touch( $dest);
    $start = $_POST['start'] - 0.22;
    $end = $_POST['end'];
    $path = $file;
    $mp3 = new PHPMP3($path);
    $mp3_1 = $mp3->extract($start,$end);
    $mp3_1->save($dest);  
